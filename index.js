...
function getYears(callback) {
  const years = [];
  callback.forEach((item) => years.push(item["Year"]));
  return years;
}
...
function getWinnersByYear(callbackw, callbacky) {
  for (let i = 0; i < callbackw.length; i++) {
    console.log(`In ${callbacky[i]}, ${callbackw[i]} won the world cup!`);
  }
}
...
function getAverageGoals(data) {
  let home = 0;
  let away = 0;
  home = data.reduce(function (accumulator, item) {
    return accumulator + item["Home Team Goals"];
  }, 0);
  away = data.reduce(function (accumulator, item) {
    return accumulator + item["Away Team Goals"];
  }, 0);
  return`Home:${home / data.length}, Away:${away / data.length}`;
}
